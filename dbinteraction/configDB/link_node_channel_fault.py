...
class Link_Node_Channel_Fault(Base):
    """
    author: Evren Keskin
    Link_Node_Channel_Fault class (link_node_channel_fault table)

    Describe a Link Node Channel Fault , which is composed of the device name,
    ...
    and specific to the link node channel fault:
    the link node id,
    card, channel
    debounce enumeration, time, and name
    is deadman
    positions x, y, and z,
    cable
    link node foreign key
    and link node card foreign key
    ...
    ### LINK NODE CHANNEL FAULT SPECIFIC:
    link_node_id: id of this link node channel fault
    card: the card of the link node
    channel: the channel the link node is on
    debounce_time: the time it takes for the debounce
    is_deadman: an additional attribute for the link node channel fault
    position_x: an additional attribute for the link node channel fault
    position_y: an additional attribute for the link node channel fault
    position_z: an additional attribute for the link node channel fault
    cable: cable of the link node

    Relationships:
    --- None

    """
    __tablename__ = 'link_node_channel_fault'
